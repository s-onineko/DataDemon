...
st.code('''
    ...
    ※予測タスクは分類タスクと比べて計算時間が長くなる傾向にあります。
    '''
    )
...
if st.checkbox('サンプルデータをダウンロードするにはチェックを入れてください'):
    ...
    st.write("サンプルデータはテストデータについても正解ラベル付きです。テスト用に使用する場合は予測したい列を削除して実施してください")
    # Download sample
...
test_size = st.slider("精度の検証用データに分割するデータのサイズを選択してください(単位：%)", 10, 90, 30, 10)/100
